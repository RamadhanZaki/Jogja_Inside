<template>
  <section class="top-hidden-gems">
    <div class="container">
      <h2 class="section-title">Hidden Gems Kulon Progo'</h2>
  ...
  methods: {
    ...
    async fetchHiddenGems() {
      try {
        ...
        const hiddenGems = response.data.data.filter(
          item => item.label === 'Hidden Gem' && item.region === 'Kulon Progo'
        )
        ...
        this.cards = hiddenGems.map(item => ({
          ...
          image: `${baseUrl}/storage/${item.thumbnail}`,
          slug: item.slug
        }))
      } catch (error) {
