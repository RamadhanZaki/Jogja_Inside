<template>
  <section class="top-hidden-gems">
    <div class="container">
      <h2 class="section-title">Destinasi Trending di Kulon Progo'</h2>
  ...
  methods: {
    ...
    async fetchTrendingDestinations() {
      try {
        ...
        const trending = response.data.data.filter(
          item => item.label === 'Trending Now' && item.region === 'Kulon Progo'
        )
