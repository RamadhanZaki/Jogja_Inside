<template>
  <section class="top-hidden-gems">
    <div class="container">
      <h2 class="section-title">Hidden Gems GunungKidul</h2>
...
export default {
  ...
  methods: {
    ...
    async fetchHiddenGems() {
      try {
        const apiUrl = process.env.VUE_APP_API_BASE || 'https://jogja-inside.my.id/api'
        ...
        const hiddenGems = allData.filter(
          item => item.label === 'Hidden Gem' && item.region === 'GunungKidul'
        )

        this.cards = hiddenGems.map(item => {
          let imagePath = 'https://via.placeholder.com/300x200?text=No+Image'

          if (item.thumbnail) {
            if (item.thumbnail.startsWith('storage/')) {
              imagePath = `${baseUrl}/${item.thumbnail}`
            } else {
              imagePath = `${baseUrl}/storage/${item.thumbnail}`
            }
          }
        ...
        console.error("❌ Gagal memuat data Hidden Gems:", error.message || error)
      }
    }
  },
  mounted() {
